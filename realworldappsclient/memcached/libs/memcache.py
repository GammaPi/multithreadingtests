...
class Client(local):
    ...
    def _get_server(self, key):
        ...
        for i in range(Client._SERVER_RETRIES):
            ...
            serverhash = serverHashFunction(
                (str(serverhash) + str(i)).encode("ascii"))
        return None, None
    ...
    def _incrdecr(self, cmd, key, delta):
        ...
        try:
            server.send_cmd(cmd)
            line = server.readline()
            if line == None or line.strip() == b'NOT_FOUND':
                return None
            return int(line)
        except socket.error as msg:
            if isinstance(msg, tuple):
    ...
    def check_key(self, key, key_extra_len=0):
        ...
        after_translate = key.translate(
            key.maketrans('', '', invalid_key_characters))
...
if __name__ == "__main__":
    ...
    for servers in serverList:
        ...
        def test_setget(key, val):
            print(
                "Testing set/get {'%s': %s} ..." % (to_s(key), to_s(val)), end=' ')
            mc.set(key, val)
            newval = mc.get(key)
        ...
        print("Testing sending a key of type bytes")
        try:
